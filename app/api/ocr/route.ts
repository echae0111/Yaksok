...
export async function POST(request: Request) {
  ...
  try {
    const body = await request.json() as { image?: string; page?: number };
    if (!body.image || body.image.length > MAX_IMAGE_LENGTH || !/^[A-Za-z0-9+/=]+$/.test(body.image)) return jsonError("OCR 페이지 이미지가 올바르지 않습니다.", 400);
    const response = await callGemini(apiKey, OCR_PROMPT, [{ text: `${body.page ?? 1}쪽` }, { inlineData: { mimeType: "image/jpeg", data: body.image } }], ocrSchema);
    if (!response.ok) {
    ...
    if (!result.text?.trim()) return jsonError("이 페이지에서 글자를 충분히 읽지 못했습니다.", 422);
    return Response.json({ page: body.page ?? 1, text: result.text });
  } catch (reason) {
